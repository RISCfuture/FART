...
struct ResultsView: View {
  ...
  ///
  /// ``animateToModel()`` steps the score through intermediate values and VoiceOver
  /// re-announces the value at each one, so this stays to a number and a unit. The risk
  /// level is its own element directly below, and the dial's normalizer is a display scale
  /// rather than a maximum, so neither belongs here.
